...
signals = []
noOfSignals = 4
simTime = 715  # change this to change time of simulation
timeElapsed = 0
...
speeds = {'car':2.75, 'bus':2.3, 'truck':2.3, 'rickshaw':2.5, 'bike':3}  # average speeds of vehicles
...
def setTime():
    ...
    speak("Detecting vehicles of arm , " + str(((currentGreen+1) % noOfSignals) + 1))
...
# Add this variable to the beginning of the code to store the last 3 green times
last_three_green_times = []


# Modify the repeat() function to update the last_three_green_times list
def repeat():
    global currentGreen, currentYellow, nextGreen, last_three_green_times
    ...
    # When a signal turns yellow, add its green time to the list
    last_three_green_times.append(signals[currentGreen].totalGreenTime)

    # If there are more than 3 green times in the list, remove the oldest one
    if len(last_three_green_times) > 3:
        last_three_green_times.pop(0)

    # Calculate the average of the last 3 green times
    avg_green_time = sum(last_three_green_times) / len(last_three_green_times)
    print("Average time for last 3 green signals:", avg_green_time)
    ...
    for i in range(0, noOfSignals):
        if i == currentGreen:
            if currentYellow == 0:
                ...
                signals[i].yellow -= 1
                if i == 3 and signals[i].yellow == 0:  # Signal 4 is yellow and about to turn red
                    print("Waiting for 10 seconds after signal 4 turns yellow...")
                    speak("Pedestrian crossing for 10 seconds")
                    time.sleep(10)  # Add 10-second delay after signal 4 turns yellow
        else:
            signals[i].red -= 1
...
def generateVehicles():
    ...
    while (True):
        # Generate a random number to determine the type of vehicle
        vehicle_type = random.randint(0, 4)  # Adjust the range for more vehicle types

        # If vehicle_type is 4, no vehicle is generated in this iteration
        if (vehicle_type == 4):
            lane_number = 0
        else:
            # Generate a random number to determine the lane number
            lane_number = random.randint(0, 1) + 0  # Adjust the range for more lanes

        # Initialize will_turn variable to 0
        will_turn = 0

        # If the vehicle is in the leftmost lane
        if (lane_number == 2):
            # Generate a random number to determine if the vehicle will turn
            temp = random.randint(0, 4)
            # Determine if the vehicle will turn left or not
            if (temp <= 2):
                will_turn = 1
            else:
                will_turn = 0

        # Generate a random number to determine the direction of travel
        temp = random.randint(0, 999)
        direction_number = 0
        a = [400, 800, 900, 1000]
        # Determine the direction based on the random number generated
        if (temp < a[0]):
            direction_number = 0
        elif (temp < a[1]):
            direction_number = 1
        ...
        # Create a new Vehicle object with the specified parameters
        Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number],
                will_turn)

        # Pause execution for a short time before generating the next vehicle
        time.sleep(0.15)
...
class Main:
    ...
    while True:
        ...
        laneOneText = font.render("CMH Hospital Road", True, black, white)
        screen.blit(laneOneText, (150, 290))
        laneTwoText = font.render("100ft Road", True, black, white)
        screen.blit(laneTwoText, (810, 100))
        laneThreeText = font.render("80ft Road Old Madras Road", True, black, white)
        screen.blit(laneThreeText, (295, 730))
        laneFourText = font.render("Halsoor Side", True, black, white)
        screen.blit(laneFourText, (1100, 555))
